...
1. Upload de map `afstort-2fa` naast de huidige map op de server.
2. Voer eenmalig `database_2fa_migratie.sql` uit op dezelfde database.
3. Open de testmap in de browser, bijvoorbeeld `https://jouwdomein.nl/afstort-2fa/login.php`.
4. Log in met een bestaand account.
5. De eerste keer wordt 2FA ingesteld. Daarna vraagt de testmap om een authenticator-code.
